# Conference Expense Planner

A modern React + Vite web app for planning conference event costs. Users can browse venue options, add AV equipment and meal selections, and view a live total cost summary for their event.

## Features

- Select a conference venue from multiple options
- Add optional AV equipment and add-ons
- Choose meal packages for attendees
- Enter the number of attendees to calculate meal costs
- View a detailed cost summary with totals by category
- Built with React, Redux Toolkit, and Vite

## Tech Stack

- React 
- Redux Toolkit
- Vite
- CSS for styling
